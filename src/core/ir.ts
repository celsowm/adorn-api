...
export function collectManifest(controllers: Function[]): ManifestIR {
  ...
  for (const ctor of controllers) {
    ...
    const routes: RouteIR[] = stubs.map((s: RouteStub) => {
      ...
      if (paramsRequired && !params) throw new RouteConfigError(`${method} ${fullPath}: params schema required`);
      if (!query) throw new RouteConfigError(`${method} ${fullPath}: query schema required (use an empty schema)`);
      if (bodyRequired && !body) throw new RouteConfigError(`${method} ${fullPath}: body schema required`);
