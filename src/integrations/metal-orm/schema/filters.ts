...
>(
  Entity: EntityCtor<T>,
  opts: Opts,
): Schema<(Opts['paging'] extends false ? Record<string, never> : Paged) & Partial<{ [K in P[number]]: HttpPick<T, K> }>> {
  ...
  const columns = table.columns as Record<string, ColumnDef>;

  for (const key of opts.pick) {
    const column = columns[key as string];
    if (!column) continue;
    shape[key as string] = optional(columnToSchema(column));
  }

  const schema = v.object(shape).strict();
  return schema as Schema<(Opts['paging'] extends false ? Record<string, never> : Paged) & Partial<{ [K in P[number]]: HttpPick<T, K> }>>;
}
