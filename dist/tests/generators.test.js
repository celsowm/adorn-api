...
describe("adorn-api generators", () => {
    it("writes swagger.json with expected routes", async () => {
        ...
        const hasTenantId = advancedGet?.parameters?.some((param) => param.name === "tenantId" && param.in === "path");
    ...
    it("writes express routes from controllers", async () => {
        ...
        const routesPath = path.join(projectRoot, "tests", "example-app", "routes.ts");
